...
本模板不再预先装载任何绘图包（如~pstricks等），完全由你自己来决定。强烈推荐《
\LaTeX2$_{\epsilon}$ 插图指南》！

\subsection{插图}
\label{sec:graphs}

使用细节请参看~\textbf{subfig} 的说明文档。

\subsubsection{一个图形}
\label{sec:onefig}
看看图~\ref{fig:xfig1} 觉得怎么样。
...
\begin{figure}[h]
  ...
  \subfloat[第一个小图形]{%
    \label{fig:subfig1}
    \includegraphics{hello.eps}}
  \subfloat[第二个小图形。如果标题很长的话，它会自动换行，并且采用一种~hang 模式，这个~caption 就是这样的例子。]{%
    \label{fig:subfig2}
    \includegraphics{hello.eps}}
  \caption{包含子图形的大图形。}
  \label{fig:big1}
