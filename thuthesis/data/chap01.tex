...
% 有一种格式定义叫做变态：研究生院说只要第一章右开就可以，以后的无所谓。肚子有点疼。
% 本科不用在乎下面这句~dirty 的设置。
\makeatletter
\ifthu@bachelor\relax\else\cleardoublepage\fi
\makeatother
...
\begin{table}[htb]
  ...
  \caption[模板文件。]{模板文件。如果表格的标题很长，那么在表格所以中就会很不美
    观，所以要像~chapter 那样在前面用中括号写一个间断的标题。这个标题会出现在索
    引中。}
